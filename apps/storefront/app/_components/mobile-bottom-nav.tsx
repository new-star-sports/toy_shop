...
export default function MobileBottomNav({ locale, user }: MobileBottomNavProps) {
  ...
  const navLinks = [
    ...
    { href: `/${locale}/search`, icon: LayoutGrid, en: "Shop", ar: "المتجر" },
    { href: `/${locale}/account/wishlist`, icon: Heart, en: "Wishlist", ar: "المفضلة" },
  ];

  const ACTIVE_THEME = "text-clay-sky-deep bg-clay-sky";

  const accountActive = pathname.includes("/account") && !pathname.includes("/wishlist");
  ...
  return (
    <>
      <nav className="sm:hidden fixed bottom-3 left-3 right-3 z-50">
        ...
        >
          {navLinks.map(({ href, icon: Icon, en, ar }) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  active ? ACTIVE_THEME : "text-muted-foreground hover:text-foreground"
                )}
              >
          ...
          {user ? (
            <Link
              ...
              className={cn(
                "flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-2xl transition-all duration-200",
                accountActive ? ACTIVE_THEME : "text-muted-foreground hover:text-foreground"
              )}
            >
