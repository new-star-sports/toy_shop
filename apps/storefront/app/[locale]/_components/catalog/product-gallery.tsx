...
export function ProductGallery({ images, locale }: ProductGalleryProps) {
  ...
  return (
    <div className="space-y-4">
      {/* Main Image */}
      <div className="relative aspect-square bg-white rounded-3xl overflow-hidden border border-border/30 group">
        <Image
        ...
        {images.length > 1 && (
          <div className="absolute inset-0 flex items-center justify-between p-4 opacity-0 group-hover:opacity-100 transition-opacity">
            <Button
              size="icon"
              variant="outline"
              className="rounded-full shadow-lg h-10 w-10 bg-white hover:bg-primary hover:text-white transition-colors"
              onClick={handlePrev}
            >
        ...
        <div className="absolute top-4 right-4">
          <Button size="icon" variant="outline" className="rounded-full shadow-sm bg-white h-8 w-8 hover:text-primary">
          ...
          {images.map((image, index) => (
            <button
              ...
              className={`relative h-20 w-20 flex-shrink-0 rounded-xl overflow-hidden border-2 transition-all ${
                activeIndex === index ? "border-primary shadow-sm" : "border-border/30 hover:border-primary/30"
              }`}
