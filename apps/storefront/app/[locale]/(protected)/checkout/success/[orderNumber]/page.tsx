  ...
  return (
    <div className="bg-nss-surface min-h-screen py-12">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Order Quick Info */}
        <div className="bg-white rounded-2xl shadow-sm border border-nss-border overflow-hidden mb-8">
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-nss-border">
            <div className="p-6 text-center">
              <p className="text-xs text-nss-text-secondary uppercase tracking-wider mb-1">{t.orderNumber}</p>
              <p className="font-bold text-nss-text-primary">#{order.order_number}</p>
            </div>
            <div className="p-6 text-center">
              <p className="text-xs text-nss-text-secondary uppercase tracking-wider mb-1">{t.date}</p>
              <p className="font-medium text-nss-text-primary">
                {new Date(order.created_at).toLocaleDateString(locale === "ar" ? "ar-KW" : "en-KW")}
              </p>
            </div>
            <div className="p-6 text-center">
              <p className="text-xs text-nss-text-secondary uppercase tracking-wider mb-1">{t.paymentMethod}</p>
              <p className="font-medium text-nss-text-primary uppercase">{order.payment_method}</p>
            </div>
            <div className="p-6 text-center">
              <p className="text-xs text-nss-text-secondary uppercase tracking-wider mb-1">{t.total}</p>
              <p className="font-bold text-nss-primary">{formatCurrency(order.total_kwd, locale)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-nss-border p-6">
            ...
            <div className="space-y-4">
              {order.order_items.map((item) => (
                <div key={item.id} className="flex gap-4">
                  <div className="w-16 h-16 bg-nss-surface rounded-lg flex-shrink-0 animate-pulse" /> {/* Image placeholder */}
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-nss-text-primary truncate">
                  ...
                  <div className="text-sm font-bold text-nss-text-primary">
            ...
            <div className="mt-6 pt-6 border-t border-nss-border space-y-2">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <Link
            ...
            className="flex-1 bg-white text-nss-text-primary border border-nss-border py-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-nss-surface transition-colors"
          >
            {t.viewOrders}
          </Link>
          <Link
